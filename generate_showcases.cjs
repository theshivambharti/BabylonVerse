...
const modules = [
    "Lighting", "Cameras", "Physics", 
    "Environment", "Effects", "Performance"
];

const template = (name) => `import { IShowcase } from "../core/interfaces/IShowcase";
...
export class ${name}Showcase implements IShowcase {
    ...
    public async load(): Promise<void> {
        ...
        const camera = CameraManager.instance.createArcRotateCamera("${name}Camera");
    ...
    private _buildUI(): void {
        ...
        title.textContent = "${name} Showcase";
        title.style.margin = "0 0 10px 0";

        const desc = document.createElement("p");
        desc.textContent = "Explore advanced ${name.toLowerCase()} features and capabilities.";
...
modules.forEach(mod => {
    const filePath = path.join(__dirname, 'src', 'showcases', mod + 'Showcase.ts');
    fs.writeFileSync(filePath, template(mod));
